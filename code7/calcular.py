    ...
    def equal(self):
        if self.operator and self.last_value is not None:
            try:
                ...
                formatted = f"{result:,.6f}".rstrip('0').rstrip('.') if not result == int(result) else f"{int(result):,}"
                font_size = 36 if len(formatted.replace(',', '')) <= 7 else 28 if len(formatted) <= 10 else 22
